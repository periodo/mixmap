...
function Map (regl, opts) {
  if (!(this instanceof Map)) return new Map(regl, opts)
  if (!opts) opts = {}
  this._com = new Component()
  this.regl = regl(self._com._element)
  this._draw = []
  this._layers = []
  this._layerTiles = []
  this._viewboxQueue = []
...
Map.prototype._setMouse = function (x, y, move) {
  const self = this
  if (!self._mouse) {
    self._mouse = [ 0, 0 ]
  } else if (move && self._size) {
...
Map.prototype._onMouse = function (ev) {
  const self = this
  const x = ev.offsetX
  const y = ev.offsetY
  const move = (ev.buttons & 1) === 1
  self._setMouse(x, y, move)
}

Map.prototype._onTouch = function (ev) {
  const self = this
...
Map.prototype.render = function (props={}) {
  ...
  if (props.mouse === false && props.touch === false) {
    this.element = html`<div class=${style} style=${cstyle}>
      <div>${this._relement}</div>
    </div>`
  } else if (props.mouse === true && props.touch === false) {
    this.element = html`<div class=${style} style=${cstyle}>
      <div onmouseover=${onMouse} onmouseout=${onMouse}
      onmousemove=${onMouse} onmousedown=${onMouse} onmouseup=${onMouse}>
        ${this._relement}
      </div>
    </div>`
  } else if (props.mouse === false && props.touch === true) {
    this.element = html`<div class=${style} style=${cstyle}>
      <div ontouchstart=${onTouch} ontouchend=${onTouch}
      ontouchcancel=${onTouch} ontouchmove=${onTouch}>
        ${this._relement}
      </div>
    </div>`
  } else if (props.mouse === true && props.touch === true) {
    this.element = html`<div class=${style} style=${cstyle}>
      <div onmouseover=${onMouse} onmouseout=${onMouse}
      onmousemove=${onMouse} onmousedown=${onMouse} onmouseup=${onMouse}
      ontouchstart=${onTouch} ontouchend=${onTouch}
      ontouchcancel=${onTouch} ontouchmove=${onTouch}>
        ${this._relement}
      </div>
    </div>`
  ...
  onload(this.element,
    ...
    () => { self._unload() })
  return this.element
  function onMouse (ev) { self._onMouse(ev) }
  function onTouch (ev) { self._onTouch(ev) }
}
